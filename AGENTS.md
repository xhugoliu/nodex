...
- 当前桌面推进的默认方向不是“继续扩工作台”，而是“收口成极简产品”
- 对默认桌面界面，优先做减法：删除、合并、降级与主路径无关的 surface
- 主要服务调试、审计、对照、开发验证的能力，默认留在 CLI / scripts / 次级入口，不应占据主舞台
...
- 先判断一个 UI 元素是否直接服务当前节点工作流；如果不是，默认不上主界面
- 能合并就不要新增并列面板；能下沉到次级入口就不要常驻；能删除就优先删除
- 不要把 run id、artifact、compare、raw payload、环境诊断、实现期状态提示回填成默认桌面内容
- Recovery、history、AI replay、provider diagnostics 这类能力可以保留，但默认应作为次级入口，而不是主舞台的一部分
