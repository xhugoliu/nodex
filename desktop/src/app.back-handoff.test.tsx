  ...
  assert.ok(
    invokeCalls.some(
      (call) =>
        ...
        call.args.start_path === "/workspace" &&
        call.args.snapshot_id === "snapshot-5",
    ),
  );
  assert.equal(requireSidePaneProps().selectionTab, "context");
  assert.equal(requireSidePaneProps().selectedSourceDetail, null);
...
test("App keeps source-context continuity when uncite draft generation fails", async () => {
  const dom = installFakeDom();
  const eventHandlers = new Map<string, (event: { payload: unknown }) => void>();
  let treePaneRenderCount = 0;
  let mainPaneRenderCount = 0;
  ...
  const bindings: Partial<AppBindings> = {
    ...
    invokeCommand: async <T,>(command: string, args: Record<string, unknown>) => {
      ...
      if (command === "draft_uncite_source_chunk_patch") {
        throw new Error("uncite draft failed: chunk-1 citation is no longer available");
      }
      throw new Error(`unexpected command: ${command}`);
    },
    openPath: async () => null,
  ...
  assert.ok(
    ...
    "uncite draft failure should still target the current node and source chunk",
  );
  assert.ok(
    treePaneRenderCount > renderCountsBeforeFailure.tree,
    "uncite draft failure should rerender the tree pane without changing selection",
  );
  assert.ok(
    mainPaneRenderCount > renderCountsBeforeFailure.main,
    "uncite draft failure should rerender the main pane without changing focus",
  );
  assert.ok(
    sidePaneRenderCount > renderCountsBeforeFailure.side,
    "uncite draft failure should rerender the side pane while keeping source context visible",
  );
