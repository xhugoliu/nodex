...
```json
{
  ...
  "rationale": "This chunk directly supports the claim."
}
```
...
- `node_id` 和 `chunk_id` 都必须存在
- 这是“显式 evidence 引用”，语义上不同于 `attach_source_chunk`
- 目标节点必须已经挂到该 chunk 所属的 source 上
- 如果 source-level link 不存在，需要先在前序 op 里 `attach_source`
- 同一个节点不能重复引用同一个 chunk
- `citation_kind` 可省略；默认是 `direct`
- 当前只接受：
  - `direct`
  - `inferred`
- `rationale` 可省略；如果提供，不能是空字符串
...
- 如果该节点对这个 source 还保留 evidence citation，也会直接报错
- 需要先在前序 op 里把对应的 `detach_source_chunk` / `uncite_source_chunk` 做完
...
- `node_id` 和 `chunk_id` 都必须存在
- 目标节点必须已经引用了这个 chunk
- 这条 op 只移除 evidence citation，不影响 `attach_source_chunk`
...
- `cite_source_chunk` 之前是否已经建立对应的 source-level link
- `detach_source` 之前是否已经移除了该 source 下的 chunk-level link 和 evidence citation
...
这版 patch 模型目前覆盖了“结构编辑 + 基础来源关联增删 + 最小 evidence 引用”。

还没有纳入 patch 的内容包括：

- 更完整的证据关系语义
- evidence quote 片段 / span
- 节点合并
- 节点拆分
- 批量重排
- 跨图引用
