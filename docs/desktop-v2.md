...
2. 在已经覆盖连续 `imported-root apply -> generated node -> patch-first/add-child/source-cite/source-uncite`
   和 source detail -> Draft / Review handoff 的基础上，
   继续补同一 mounted session 内生成节点上的 `source detail -> Draft -> review/apply` 闭环
