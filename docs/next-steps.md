...
- 当前桌面壳已经从“调试 / 审计工作台”进一步收成一版最小节点工作流
...
### 3. 为桌面 v2 打开 React Flow 画布切口

状态：与第一项并行推进，但不抢主流程收口

目标：

- 把 `React Flow` 固定为桌面 v2 默认画布层
- 先验证“同一份节点树 -> 画布投影 -> 选中同步 -> apply 后聚焦”这条最小闭环
- 保持画布层继续只做交互与呈现，不接管 canonical state

当前最小切口：

- 从现有 tree / node context 查询结果生成 `React Flow` nodes / edges
- 大纲选中和画布选中双向同步
- apply 后自动把视口和焦点带到新增节点
